...
# sleep_sec = 30
# date = None
# scraper.daily_total_return_index_update(sleep_sec, date)
...
class DailyScraper:
    ...
    # (2) Retreive table not up-to-date
    def get_outdated_tables(self) -> List[str]:
        today_date = pd.Timestamp.today().strftime('%Y/%m/%d')
        print(f"Today's date:{today_date}")
        sql = """   SELECT table_name
                    FROM latest_updated_date
                    WHERE frequency = %s AND \
                    latest_date != %s;
                    """
        try:
            # connect to the PostgreSQL database
            ...
            cur = conn.cursor()
            # execute the SQL statement
            cur.execute(sql, ('daily', today_date))
            rows = cur.fetchall()
    ...
    def insert_function(self, df, table):
        conn = psycopg2.connect(**self.conn_params)
    ...
    def update_latest_date(self, date, table):
    ...
    #* Scrape Functions
    # (1)  Update daily stock price information (OHLC data) into "daily_stock_price" table
    def daily_stock_price_update(self, sleep_sec: int, to_date = None) -> None:
        ...
        def daily_scrape_listed(scrape_date):
            ...
            if isinstance(content, pd.DataFrame):
                ...
                print(f'The listed data on {scrape_date_str} is not available')
                return None
        # 6. Scrape OTC
        def daily_scrape_otc(scrape_date):
            ...
            def scrape_unit(url, retry_times = 3):
                ...
                while retry_times >= 0:
                    try:
                        res = requests.get(url, headers=headers)
                        content = res.json()
                        check_status = content['aaData']
                        if len(check_status) == 0:
                            return None
                        return_df = pd.DataFrame(check_status)
                        return return_df
                    except (requests.ConnectionError, requests.ReadTimeout) as error:
                        print(error)
            ...
            if isinstance(content, pd.DataFrame):
                ...
                print(f'The otc data on {scrape_date_str} is not available')
                return None
        # 7. Organized data
        ...
        # Execution
        target_table = 'daily_stock_price'
        current_date = self.get_last_date(target_table)
        closed_date_list = self.get_closed_date()
        ...
        for date in tqdm(date_list):
            ...
            if (isinstance(listed_df, pd.DataFrame) and isinstance(otc_df, pd.DataFrame)):
                ...
                execution_time -= 1
                print(f"{date} data is unavailable")
    
    # (2) Update daily three institution buying information into "daily_institution_trade" table
    def daily_three_insti_buying_update(self, sleep_sec: int, to_date = None) -> None:
        # 3. Create scraped date
        def create_scrape_date(current_date, closed_date_list, specified_date = None):
            if specified_date:
                to_date = pd.to_datetime(specified_date)
        ...
        def organized_scraped_data(listed_df, otc_df, stock_id_list):
            ...
            return export_df
        # 10. Execute update
        target_table = 'daily_institution_trade'
        current_date = self.get_last_date(target_table)
        closed_date_list = self.get_closed_date()
        date_list = create_scrape_date(current_date, closed_date_list, specified_date = None)
        start_date = date_list[0].strftime('%Y-%m-%d')
    ...
    # (3) Update daily total transaction information(value, volume, turnover) from List and OTC buying information 
    #  into "daily_total_trade" table
    def daily_total_trade_update(self, sleep_sec: int, to_date = None) -> None:
        ...
        # Execution
        target_table = 'daily_total_trade'
        start_date = self.get_last_date(target_table)
        ...
        for index in tqdm(range(content_len)):
            ...
            self.insert_function(inserted_df, target_table)
            newest_date = inserted_df.Date.max()
            if pd.notna(newest_date):
                self.update_latest_date(newest_date, target_table)
            content_len -= 1
            if content_len > 0:
                time.sleep(sleep_sec)

    # (4) Update daily three institution total buying information into "daily_institution_total_trade" table
    def daily_institution_total_trade_update(self, sleep_sec: int, to_date = None) -> None:
        # 3. Create scraped date
        def create_scrape_date(current_date, closed_date_list, specified_date = None):
            if specified_date:
                to_date = pd.to_datetime(specified_date)
        ...
        # 8. Execute update
        target_table = 'daily_institution_total_trade'
        current_date = self.get_last_date(target_table)
        closed_date_list = self.get_closed_date()
        date_list = create_scrape_date(current_date, closed_date_list, specified_date = None)
        start_date = date_list[0].strftime('%Y-%m-%d')
    ...
    # (5) Update daily List and OTC index information into "daily_total_return_index" table
    def daily_total_return_index_update(self, sleep_sec: int, to_date = None) -> None:
        # 2. Create listed month index
        def month_generator_listed(start_date, specified_date = None):
            if specified_date:
                to_date = pd.to_datetime(specified_date)
        ...
        # Execution
        target_table = 'daily_total_return_index'
        start_date = self.get_last_date(target_table)
        month_list = [month_generator_listed(start_date, to_date), 
                    month_generator_otc(start_date, to_date)]
        content_len = len(month_list[0])
